...
#Clase Princpial
#VentanaMenu esta instanciando QMainWindow
#QMainWindow establece una ventana principal para arrancar
class VentanaMenu(QMainWindow):
    # Constructor, parent es usado para tranferir informacion entre clases
    def __init__(self, parent = None, *args):
        super(VentanaMenu, self).__init__(parent = None)
        #Establecer el tamaño de la ventana
        #self.setMiniumSize(500,300)
        #self.setMaximumSize(500,300)
        self.setFont(QFont('arial', 20))
        # Parametros ancho y alto estatico
        self.setFixedSize(1280,720)
        # Asignar titulo a la ventana
        self.setWindowTitle("Menu")
        #Crear label("titulo",self para la ubicacion dentro de la clase VentanaMenu)
        background = QLabel(self)
        #Estilos para el label
        #label.setStyleSheet("background:#424242; colo:#fff")
        #Expandir el label al tamaño de la ventana principal
        #self.setCentralWidget(label)
        #Mover el contenido del label
        #label.setAlignment(Qt.AlignRight)
        #label.setAlignment(Qt.AlignLeft)
        #label.setAlignment(Qt.AlignHCenter)
        #label.setAlignment(Qt.AlignLeft | Qt.AlignVBottom)
        #label.setAlignment(Qt.AlignLeft | Qt.AlignBottom)
        #label.setAlignment(Qt.AlignHCenter| Qt.AlignBottom)
        # label.setAlignment(Qt.AlignCenter)
        background1 = QPixmap(back).scaledToWidth(1400)
        #Esteblece posicion de izquierda a derecha, de arriba hacia abajo, ancho y alto
        background.setGeometry(0,0,background1.width(),background1.height()-40)
        background.setPixmap(background1)
        #Obtener resolucion de elementos
        #print("ancho",label.frameGeometry().width(),"alto",,label.frameGeometry().height())

        #Crear button
        #sel.btn = QPushButton("Presioname",self)

        #Primera forma
        #self.btn.clicked.connect(self.slot)

        #Segunda forma
        #mensaje = "Este dato ha sido enviado por una signal y recibido por un slot"
        #self.btn.clicked.connect(lambda: self.slot(mensaje)) o self.btn.clicked.connect(lambda x: self.slot(mensaje))
        #def slot(self):
            #print("Se ha hecho un click")

        #Tercera forma
        # def slot_with_params(self,mensaje)
        # print(mensaje )
        # def single.slot(self,mensaje)
        # print(mensaje )
    ...
    def AbrirAnadir(self):
        ...
        window.show() #muestra la ventana principal(por default estan ocultas)
